...
def analyze_fluency(data, rate, text, wav_path):
    ...
    min_intensity = np.min(np.abs(data))
    ...
    # Output the fluency metrics with ranges
    ...
    print(
        f"Average Pause Duration: {np.mean(pauses):.2f} seconds (Range: {min(pauses):.2f} - {max(pauses):.2f} seconds)" if pauses else "Average Pause Duration: No pauses detected")
    print(f"Filler Words Count: {filler_count}")
    print(f"Jitter: {jitter:.2f}")
    print(f"Shimmer: {shimmer:.2f}")
    print(f"Average Speech Intensity: {speech_intensity:.2f} (Range: {min_intensity:.2f} - {max_intensity:.2f})")
    print(f"Syllable Count: {syllable_count}")
    print(f"Syllables per Second: {syllables_per_second:.2f}")
    print(f"Mean Pitch: {mean_pitch:.2f} Hz (Range: {min_pitch:.2f} - {max_pitch:.2f} Hz)")
